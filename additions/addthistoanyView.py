    ...
    for e in ev:
        ...
        for a in range(MinAns,MaxAns+1):
            ansHtml+='<input  type="radio" id="a'+str(a)+'" name="'+str(e.id)+'" value="'+str(a)+'" onclick="sendFeedback('+str(e.id)+','+str(a)+','+str(evcounter)+')"/><label  for="a'+str(a)+'">'+ansemojies[a-MinAns]+'</label>'
